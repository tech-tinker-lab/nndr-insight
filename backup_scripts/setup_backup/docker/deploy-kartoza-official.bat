...
scp setup\docker\docker-compose.official-kartoza.yml %REMOTE_USER%@%REMOTE_HOST%:%REMOTE_DIR%/

echo ✅ File transfer completed

REM Clean up and start on remote server
echo 🔧 Starting official Kartoza PostGIS on remote server...
ssh %REMOTE_USER%@%REMOTE_HOST% "cd %REMOTE_DIR% && docker compose -f docker-compose.official-kartoza.yml down -v && docker compose -f docker-compose.official-kartoza.yml up -d"
