...
# Build script for custom PostGIS ARM64 image
set -e

echo "🚀 Building custom PostGIS ARM64 image for NNDR Insight..."
...
# Set build arguments
IMAGE_NAME="nndr-postgis-arm64"
IMAGE_TAG="latest"
FULL_IMAGE_NAME="${IMAGE_NAME}:${IMAGE_TAG}"

print_status "Building PostGIS image: $FULL_IMAGE_NAME"

# Build the image
print_status "Starting build process (this may take 10-15 minutes)..."
docker build \
    --platform linux/arm64 \
    --file Dockerfile.postgis-arm64 \
    --tag $FULL_IMAGE_NAME \
    --progress=plain \
    .

if [ $? -eq 0 ]; then
    print_success "PostGIS image built successfully!"
    ...
    echo ""
    print_status "Or use the custom docker-compose file:"
