...
def create_table_schema(cur):
    ...
    # Drop existing table
    cur.execute("DROP TABLE IF EXISTS os_open_names_sample;")
    ...
    create_sql = f"""
    CREATE TABLE os_open_names_sample (
        {', '.join(column_definitions)}
    );
    """
    
    cur.execute(create_sql)
    print(f"Created sample table with {len(OS_OPEN_NAMES_COLUMNS)} official columns + geometry")
    ...
    try:
        with psycopg2.connect(
            ...
            with conn.cursor() as cur:
                print("Creating sample table with official columns...")
                ...
                for file_path in csv_files:
                    ...
                    try:
                        ...
                        if first_row:
                            ...
                            insert_sql = f"""
                                INSERT INTO os_open_names_sample ({', '.join(['"' + col + '"' for col in OS_OPEN_NAMES_COLUMNS])})
                                VALUES ({placeholders})
                            """
                ...
                cur.execute("SELECT COUNT(*) FROM os_open_names_sample")
                row = cur.fetchone()
                final_count = row[0] if row else 0
                
                cur.execute("SELECT COUNT(*) FROM os_open_names_sample WHERE geom IS NOT NULL")
                row = cur.fetchone()
                geom_count = row[0] if row else 0
                
                print(f"✅ Successfully loaded {final_count} sample rows into os_open_names_sample")
